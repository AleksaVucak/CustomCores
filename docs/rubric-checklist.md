...
**Last updated:** Stage 15 / Commit 15.9 (resolve final rubric and usability defects — Stage 15 complete)
...
| **15.9** | Resolve final rubric and usability defects — new [`docs/final-defect-fixes.md`](final-defect-fixes.md). Ran a four-part closing sweep: (1) **static** — grep for `TODO`/`FIXME`/`BUG`/"not implemented" (0 genuine markers; all hits are `placeholder=` attrs, `is_debug()` guards, or SQL `$placeholders`) + `php -l` on **all 94** PHP files (0 syntax errors); (2) **runtime** — started a dev server with `error_reporting=E_ALL`, `log_errors=1`, `display_errors=0` and crawled **every** page (30 public + edge cases: valid/nonexistent/non-numeric product ids, `'"<>` search injection probe, all `api/*`, a missing path; plus 25 authenticated customer + admin pages via a disposable DB admin logged in over HTTP with cookie jar + CSRF) → **0 notices/warnings/deprecations/fatals, 0 error leakage**, all status codes correct; (3) **usability** — status-code + edge-case review. **1 real defect found + fixed:** nonexistent/invalid products returned HTTP **200** with a friendly "not found" message (a "soft 404") — `product.php` now distinguishes a genuinely missing/invalid/disabled product (real **404**, full styled shell retained) from a transient DB error (stays 200); verified `id=1`→200 while `?id=999999`/`abc`/`0`/`-5`/none→404. Non-defects documented (API `405` on bare GET, `303` guards for foreign/nonexistent record ids, and the `php -S` missing-file→homepage fallback that does not occur on real Apache/Nginx hosting). Disposable admin + cookie jar deleted (store back to baseline). **Result: no known critical or rubric-blocking issue remains — Stage 15 (Testing & QA) complete.** Only #11 (live hosting) remains, prepared and deferred to Stage 16 by design. |
| **15.8** | Complete the 100-point rubric audit — new [`docs/rubric-audit.md`](rubric-audit.md): a **requirement → page + file + test** evidence table for every Section A point, re-verified against the running project and working tree. Live counts taken directly from the DB/filesystem: **20** active products with **0** having < 2 options (323 options), **3** theme CSS files + 3 DB rows, **7** Help pages, **33** images (20 product + 13 site), **3** media files (2× MP4 + 1× MP3), **50** purposeful dynamic PHP files (30 public/customer + 17 admin + 3 API), and all four SEO assets present. HTTP smoke test: 19/19 public + SEO URLs and 7/7 Help pages returned **200**; `profile`/`cart`/`checkout`/`admin` correctly **303 → login**. Behavioural points cite the earlier Stage 15 records (15.1 HTML, 15.2 CSS, 15.3 JS/console, 15.4 desktop, 15.5 mobile, 15.6 customer, 15.7 admin). **#10a promoted In progress → Complete** (page count settled at 50). **Result: 98/100 points fully evidenced *and* tested; the remaining 2 (#11 live hosting) have their page slot + deployment docs ready but the live-load test is Blocked until Stage 16** — the only rubric item whose completion test cannot pass locally, stated honestly. Every one of the 100 points now maps to a real page and a real file. Next: 15.9 resolve any final rubric/usability defects. |
| **15.7** | Verify complete administrator workflows — new [`docs/admin-workflows.md`](admin-workflows.md). Drove the running site over `php -S` with a cookie-jar + CSRF end-to-end harness through the full **product → monitoring** journey and **every core admin action**, verifying each by both its `303` PRG redirect **and** the resulting DB row change: admin **access guard** (guest → `login.php`, logged-in customer → `profile.php`, admin allowed), dashboard, products (list/search/filter, **add with image upload**, edit/update, disable, enable), product options (create / set-default / toggle / delete), compatibility (toggle a rule + restore, `update_rule`), orders (`update_status`, `update_notes`), users (disable/enable, promote customer→admin, demote admin→customer), consultations (`save_response` auto-advancing to *answered*, `update_status`), reviews (approve, hide), reports (Chart.js payloads present), monitoring (health checks), and themes (activate a different theme + restore the original). The **self-lockout** guard was verified live (an admin's own `update_role` is rejected; last-active-admin protection additionally covered by `includes/admin-users.php`). Because no admin is seeded (by design), the harness registered a disposable customer + admin (promoted in DB) and had the customer create an order, a pending review, and a consultation for the admin to manage. **Result: 51/51 assertions Pass, 0 defects — no source changes required.** All test data (product + uploaded image, both users, order/review/consultation) deleted and global settings (theme, toggled rule) restored; store verified back to baseline (20 active products, 0 orders, seed review mix 8/1/1, 7/7 rules active, `active_theme_id=1`, no orphan uploads). Satisfies "every core administrator action succeeds." Next: 15.8 rubric audit. |
| **15.6** | Verify complete customer workflows — new [`docs/customer-workflows.md`](customer-workflows.md). Drove the running site over `php -S` with a cookie-jar + CSRF end-to-end harness through the full **registration → order** journey and **every core customer action**: register (+ duplicate-email rejection), login (+ wrong-password rejection), authenticated session, catalogue browse/sort/filter, search, product detail, compare, wishlist add/view/remove, custom PC builder step-walk (fed a **complete + compatible** component set computed via the app's own `customcore_compatibility_check()`), live-price API, save build, saved-build → cart, cart add (with options, qty>1)/update, checkout with validated shipping+payment, **order creation** (PRG → `order-confirmation.php?id`), order confirmation/history/details, foreign/non-existent order rejection, review submission (enters `pending` moderation), consultation request + history, edit-profile details, password change + re-login, and logout. **Result: 41/41 assertions Pass.** **Avoidable defect found + fixed:** ordering a **saved build** failed — `customcore_snapshot_build()` in `order-confirmation.php` joined `component_categories` on the non-existent `c.category_id` (the `components` FK is `component_category_id`; `category_id` belongs to `products`), throwing `SQLSTATE[42S22]` that the order-creation `try/catch` swallowed and rolled back, so no order with a saved build could ever be placed; corrected to `c.component_category_id` (matching every other components join) — a saved build now orders and stores a valid `build_snapshot_json` (~692 B). Disposable `@example.test` customer used; all test users/orders/builds/carts/consultations/reviews deleted afterward (store back to 3 seed customers, 0 orders/builds/carts). Satisfies "every core customer action succeeds." Next: 15.7 administrator workflows. |
| **15.5** | Mobile responsiveness testing — new [`docs/responsiveness-mobile.md`](responsiveness-mobile.md). Drove the project over `php -S` in a real Chromium tab using a cache-busted fixed-width **iframe harness** and, at **320 / 360 / 390 / 414 / 700 / 768 px**, ran an **ancestor-aware** overflow probe (`documentElement.scrollWidth` vs `innerWidth`, plus a per-leaf check that filters out intended `overflow-x` scroll wrappers **and** `overflow:hidden` clips such as the Leaflet map) across the same **15 public**, **11 customer**, and **14 admin** page states as 15.4. Signed-in + admin coverage used a disposable customer briefly promoted to admin via a temporary DB helper, then the account, its order, its cart, and the helper file were **deleted** (store returned to pre-test state: three seed customers, 0 orders, 0 carts). **Result after fixes: 40/40 Pass at every width; 9 avoidable horizontal-overflow defects found and fixed** — (1) builder performance-chart `<canvas>` forced fluid (`.perf-chart__canvas-wrap > canvas { width/height:100% !important }`); (2) catalogue stats table wrapped in `.catalogue-chart__table-wrap` (`overflow-x:auto`) + `.catalogue-chart` `minmax(0,1fr)`; (3) `.layout-split` → `minmax(0,1fr)` and `.cart-table { min-width:0 }` in the ≤640 card-view; (4) `.profile-details__list dd { overflow-wrap:anywhere }` for long emails; (5) responsive order-confirmation totals footer (wide colspan-3 / narrow colspan-2 rows toggled by media query) fixing a phantom-column misalignment; (6) `.admin-table__sub { overflow-wrap:anywhere }`, `.admin-activity__grid { minmax(0,1fr) }` + dashboard tables wrapped in `.admin-table-wrap`; (7) `.admin-dl` shrinkable columns + `dd { overflow-wrap:anywhere }`; (8) monitoring table wrapped in `.admin-table-wrap`; (9) reports charts `minmax(0,1fr)` + `.admin-report-users` `minmax(min(280px,100%),1fr)`. Two ~13 px `scrollWidth` readings (profile, admin order-details at 320 px) were confirmed to be the **iframe vertical-scrollbar artifact** (ancestor-aware scan: 0 real offenders), and Leaflet tiles are clipped by `overflow:hidden` — both recorded as non-defects. Completes the mobile half of **B15**; with 15.4, **B15 Complete**. |
| **15.4** | Desktop responsiveness testing — new [`docs/responsiveness-desktop.md`](responsiveness-desktop.md). Drove the project over `php -S` in a real Chromium tab and, at **1024 / 1280 / 1440 / 1920 px**, ran an automated overflow probe (`documentElement.scrollWidth` vs `innerWidth`, plus a per-element check that filters out intended `overflow-x` scroll wrappers) across **15 public** (home, catalogue, product guest+auth, builder, 4-way compare, about, store-locations map, reviews, contact, search, login, register, media, accessibility, `help/index.html`), **11 customer** (profile, edit-profile, saved-builds, wishlist, order-history empty+populated, order-details, order-confirmation, cart, checkout, consultation, consultation-history), and **14 admin** (dashboard, products, orders, users, reviews, reports charts, monitoring, themes, compatibility, product add/edit/options, order-details, user-edit) page states. Signed-in + admin coverage used a disposable customer (registered → added to cart → placed an order to populate order views) briefly promoted to admin via a temporary DB helper, then the account, its order, and the helper files were **deleted** (store returned to pre-test state). **Result: 40/40 Pass, 0 unintended horizontal overflow, 0 clipped/broken layouts, 0 defects — no CSS changes required.** Confirmed the shell caps at **1120 px** (`--cc-width-max: 70rem`) and centres on wide screens (≈400 px gutters at 1920, ≈160 px at 1440) and that wide admin tables + the comparison table scroll **inside their cards** by design (`overflow-x:auto` wrappers), never the page. Satisfies the desktop half of **B15** (mobile completed in 15.5). |
| **15.3** | Verify JavaScript and browser console — new [`docs/js-validation.md`](js-validation.md). Static-checked all **11** modules in `assets/js/` with `node --check` (**0 failures**). Chromium headless console + `pageerror` / uncaught-listener sweep of public, Help, customer, and admin core pages with feature interactions (builder radio→live price/compat/chart, contact “Other”, help hub filter, cart qty steppers, checkout validation, review form validation, Leaflet map, admin report charts): **0 application uncaught errors**. Conditional loads confirmed intentional (e.g. `reviews.js` only when `$loadReviewForm`, `help-hub.js` only on the hub). **Avoidable defect fixed:** Leaflet 1.9.4 CSS SRI in `includes/header.php` did not match the CDN payload and blocked the stylesheet with a console integrity error; updated to the real digest `sha256-p4NxAoJBhIIN+hmNHrzRCf9tD/miZyoHS5obTRR9BMY=` and set `crossorigin="anonymous"` on Leaflet CSS/JS in header/footer. Post-fix: locations loads map scripts with a clean console. Satisfies “core features free of uncaught JS errors; defects corrected.” |
| **15.2** | Validate CSS files — new [`docs/css-validation.md`](css-validation.md). Offline suite over all five project stylesheets (`assets/css/main.css`, `assets/css/admin.css`, `assets/themes/{rgb-gaming,minimal-pro,cyber-grid}.css`): structural brace/comment balance, full AST parse via css-tree (**0 parse errors**, no empty rulesets), dangerous-legacy scan (`expression`/`behavior`/`progid` — none; only modern `scroll-behavior`), and a **custom-property graph** (every `var(--…)` use vs declaration across the style system). **Avoidable defects fixed:** seven undeclared tokens used by builder steps, Help centre, soft status fills, and admin links — declared on `main.css` `:root` as semantic aliases (`--cc-color-primary` → accent, `--cc-color-muted` → text-muted, `--cc-color-surface` → bg-elevated, `--cc-color-link` → accent) plus soft fills (`success-bg`, `warning-bg`/`warning-text`); dark themes override soft fills for near-black surfaces; Minimal Pro retunes light soft fills; `.results-estimates__bar` defaults `--score: 0` (runtime still set inline on builder-results). Post-fix re-run: **zero undefined custom properties**. W3C online validator noted unreachable from the agent environment; offline parse is the recorded gate. Satisfies “avoidable CSS errors are corrected.” |
| **15.1** | Validate rendered HTML output — new [`docs/html-validation.md`](html-validation.md). Captured real server-rendered HTML over `php -S` for important **public** (home, about, catalogue, product detail, search, compare, builder, reviews, contact, media, store-locations, accessibility, login, register), **Help centre** (`help/*.html`), **customer** (profile, edit-profile, cart, wishlist, saved-builds, order-history, consultation, consultation-history), and **administrator** (dashboard, products/add/edit/options, orders, users/edit, consultations, reviews, compatibility, reports, themes, monitoring) pages. Two automated suites: (1) core structural rules — DOCTYPE, `html[lang]`, title/charset/viewport, header/main/footer landmarks, **exactly one `h1`**, unique ids, every `img` has `alt`, every `label[for]` resolves, every POST form carries `_csrf`, no PHP error leakage; (2) HTML Tidy Errors with `--doctype html5`. **Result: 48 Pass · 0 Fail; zero Tidy Errors.** Tidy “missing dd/dl” on themes/builder/profile/store-locations are false positives for the valid HTML5 `<dl><div><dt>/<dd></div></dl>` grouping pattern (recorded, not rewritten). No markup source changes required (Stages 1–14 already establish the shared shell). Satisfies “important rendered pages have no major markup errors.” |
| **14.10** | File upload security audit (audit + hardening; extends [`docs/security-audit.md`](security-audit.md) §6). Audited both upload surfaces — admin product images (`includes/admin-products.php`) and customer consultation attachments (`includes/consultations.php`) — against type/MIME/size/name/storage. Both detect the real type with `finfo` (`FILEINFO_MIME_TYPE`) and match a **MIME allowlist** (product: jpg/png/webp/gif; consultation: pdf/txt/png/jpg/webp; **SVG excluded** to block script-carrying images), derive the stored extension from the detected MIME (client extension/type never trusted, **fails closed** if finfo missing), cap size at `upload_max_bytes` (2 MB) and — for consultations — count at `CUSTOMCORE_CONSULTATION_MAX_FILES` (5), reject empty/`UPLOAD_ERR` files, store under a `bin2hex(random_bytes(16))` name via `is_uploaded_file()` + `move_uploaded_file()` (`chmod 0644`), and keep the sanitized original name for display only. Serving is confined to `consultation-attachment.php` / `admin/consultation-attachment.php`: login/ownership (or admin) check, generic 404 (no enumeration), `ctype_digit` id, `basename` + `realpath` confinement to the upload dir, `Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`, RFC 5987 filename, `Cache-Control: private, no-store`. **Verified** with the app's real finfo+allowlist logic that a PHP webshell (even renamed `.jpg` → detected `text/x-php`) and an SVG-with-`<script>` (`image/svg+xml`) are **rejected**, while valid PDF/TXT/PNG are accepted with the correct derived extension. **Hardening added:** `uploads/products/.htaccess` (`Options -Indexes`, disables PHP engine, denies script-like extensions; images still served) and `uploads/consultation/.htaccess` (`Options -Indexes`, denies all direct web access since files are delivered only via the endpoints) — both Apache 2.2/2.4-safe, no-ops off-Apache, tracked via `.gitignore` exceptions. Satisfies the "type/MIME/size/name/storage checks; invalid and dangerous files rejected" acceptance criterion. **Stage 14 acceptance met** (SEO · a11y · comments · prepared statements · output escaped · CSRF · uploads protected). |
| **14.9** | CSRF protection on all state-changing requests (audit + fix; extends [`docs/security-audit.md`](security-audit.md) §5). Confirmed the token system (`includes/csrf.php`: 32-byte `random_bytes` token, `hash_equals()` verify, escaped `_csrf` hidden field) is applied everywhere: cross-referenced **every** `<form method="post">` against `customcore_csrf_field()` (per-file counts match) and **every** POST handler against a `customcore_csrf_verify()` that rejects missing/invalid tokens — either a "session expired" flash+redirect (`cart`, `wishlist`, `reviews`, `saved-build`) or a blocking error branch (`login`, `register`, `checkout`, `consultation`, `builder`, `builder-results`, `edit-profile`, `contact`, and all `admin/*` writes). The read-only `api/` endpoints (`builder-price`, `compatibility-check`, `chart-data`) do no writes, so they are not state-changing. **Gap found + fixed:** `logout.php` was reachable by GET via a plain nav link (logout-CSRF, e.g. `<img src="logout.php">`); it now performs logout **only** on a token-verified POST (GET / missing / wrong token → redirect, no logout), and the logout controls in `includes/navigation.php` + `includes/account-nav.php` are CSRF token POST forms with button-as-link styling in `assets/css/main.css`. Verified over HTTP (GET and token-less POST → 303 without logout) and via the guard truth table (only POST+valid token logs out). Satisfies the "tokens on all state-changing forms; invalid rejected" acceptance criterion. |
| **14.8** | Security audit — prepared statements & output escaping (new [`docs/security-audit.md`](security-audit.md)). Enumerated every DB call (`->query`/`->exec`/`->prepare`; 0 `->exec`) and confirmed each is a static SQL literal or a bound prepared statement. Reviewed every dynamic SQL fragment: `WHERE` builders append only fixed clause literals with bound values; `IN (...)` lists use generated placeholders with bound IDs (`compare.php`, `builder-results.php`, `includes/performance.php`, `api/builder-price.php`); `LIMIT`/`OFFSET` use `int`-typed, clamped values (`max(5, min(100, …))`, offset from a clamped page) in all four admin listers; dynamic `SET` writes use whitelisted columns (`admin-products`, `admin-compatibility`); the one interpolated identifier `{$table}` is regex-whitelisted and only called with internal literals. Output: `customcore_e()` (`htmlspecialchars`, `ENT_QUOTES | ENT_SUBSTITUTE`) wraps all dynamic HTML; the only raw `echo`s are boolean/ternary literals, `int`-cast numbers, pre-escaped strings, or XML-escaped sitemap output; no superglobal is echoed. Client JS escapes via a textNode `escapeHtml()` before `innerHTML` (`builder.js`, `charts.js`) or builds with `textContent` (`store-map.js`). Also confirmed open-redirect/header-injection guards (`customcore_is_safe_return_target()` / `customcore_is_safe_local_path()`). Result: **PASS, 0 vulnerabilities, no source changes**; doc includes a repeatable `rg` recipe. Satisfies the "prepared statements · output escaped" stage-acceptance criteria. |
| **14.7** | JavaScript & PHP documentation — closed out the code-comment stage (rubric #5/#6). JS: every file in `assets/js/*` already had a file-responsibility header; this commit adds JSDoc (purpose + `@param`/`@returns`) to all remaining named functions — `contact.js`, `catalogue-chart.js`, `store-map.js`, `admin-reports.js`, `help-hub.js`, plus the nested `step()` in `cart.js` and the `boot()` entry points in `builder.js`/`charts.js` (main.js/checkout.js/reviews.js were already complete). PHP: a temporary token-based audit (`token_get_all`, counting `T_FUNCTION` with a preceding `T_DOC_COMMENT`, allowing visibility modifiers) scanned the whole tree and found only 3 undocumented functions of 282 — `catalogue_filter_url()` and the `customcore_flash_warning()`/`customcore_flash_error()` helpers — now documented; a re-run reports 0 undocumented and 0 files missing a header. Verified: `node --check` on all 11 JS files and `php -l` on the changed PHP files are clean. Stage 14 comment pass (14.6 + 14.7) complete; #5 and #6 Complete. |
| **14.6** | HTML & CSS comments — completed the markup/style documentation pass for rubric #6. CSS was already fully commented (numbered section headers and dividers in `assets/css/main.css`, `assets/css/admin.css`, and all three themes, which carry rich file-responsibility headers plus 120–150 section dividers each); this commit adds structured, purpose-first `<!-- section -->` comments to the HTML/output portion of every view template — 25 public/customer pages (`index`, `catalogue`, `product`, `builder`, `builder-results`, `compare`, `search`, `cart`, `checkout`, `order-confirmation`, `order-history`, `order-details`, `login`, `register`, `profile`, `edit-profile`, `wishlist`, `saved-builds`, `saved-build`, `about`, `accessibility`, `media`, `store-locations`, `contact`, `reviews`, `consultation`, `consultation-history`) and 16 admin pages (`index`, `products`, `product-add`, `product-edit`, `product-options`, `orders`, `order-details`, `users`, `user-edit`, `consultations`, `consultation-details`, `compatibility`, `reviews`, `reports`, `monitoring`, `themes`). Comments mark major landmarks (hero, filters, results grids, tables, forms/fieldsets, KPI cards, chart panels, flash/empty-state branches) and explain purpose rather than restating tags, placed on their own line before the block and never inside a tag or between a control structure and its body. Verified additions-only via `git diff --numstat` (0 deletions across 41 files) and a full `php -l` sweep (all clean). #6 In progress (JS/PHP remain in 14.7). |
| **14.5** | Advanced CSS interactions — layered a cohesive, additive polish pass on top of the already-advanced foundation (auto-fit/auto-fill responsive grids, `aspect-ratio` media, `clamp()` type, tokenised transitions, three themes). New in a dedicated "Advanced CSS interactions" section of `assets/css/main.css`: (1) **cards** — a `:focus-within` elevation so keyboard users get the same lift as `:hover`, using the themed `--cc-color-focus` border + `--cc-shadow-md`; (2) **menus** — the mobile nav panel now animates in via a `@keyframes cc-nav-reveal` (opacity + translate/scaleY) since `display` cannot transition; (3) **transitions** — a smooth `--cc-color-accent` label transition; (4) **form states** — `.form-row:focus-within > .form-label` highlights the active field's label; (5) **advanced technique** — hover-only deepening of the card lift gated behind `@media (hover: hover) and (pointer: fine)` so touch devices never get a stuck hover. All effects use `--cc-*` tokens so the three themes inherit them (theme `.card:hover` rules still win where defined), and both reduced-motion blocks neutralise the timing. Verified: no lint, braces balanced, keyframe name unique across themes. Supports #12. |
| **14.4** | Accessibility & keyboard navigation — hardened the core-flow experience for keyboard and screen-reader users. Confirmed the existing foundation site-wide: a `.skip-link` to `main#main-content`, a global `:focus-visible` ring (plus per-component focus styles), every form control paired with a `<label for>` / labelled search + filter regions, `aria-invalid` + `aria-describedby` on errored fields, and flash messages announced via `aria-live` with `role="alert"` (errors) / `role="status"` (success/warning). New in 14.4: after a failed submit, `assets/js/main.js` (`initErrorFocus`) moves focus to the first `aria-invalid` field — or, on POST-form pages, a form-level error alert — and scrolls it into view, so problems are announced and reachable without a mouse (respects intentional in-page anchors; complements the pre-submit validation in `checkout.js`/`reviews.js`/`contact.js`). Added a `@media (prefers-contrast: more)` block that thickens the focus ring (outline-only, no layout shift). Verified: `node --check` clean, no lint, reduced-motion + increased-contrast preferences honoured, and full register/login/checkout flows operable by keyboard alone. Completes Stage 14. |
| **14.3** | Semantic HTML structure — audited landmarks and heading hierarchy across all pages. Shared shell already provides `header[role=banner]`, labelled `nav`, `main#main-content` (skip-link target), and `footer[role=contentinfo]`; content pages use `section`/`article` with `aria-labelledby`. Fixed the only heading defect: `admin/reports.php` skipped `h1 → h3` on its KPI cards — the summary block is now a labelled `<section>` with an `<h2>` ("At a glance") so stat labels nest correctly as `h3` (CSS wrapper class added to `admin.css`, layout unchanged). Verified: `php -l` clean, static heading audit, and a rendered check of `admin/reports.php` (`h1 → h2 → h3`, one `h1`, zero skips); builder / build-summary / order-confirmation carry two source `h1`s only across mutually-exclusive branches (confirmed one renders at runtime). #13 Complete |
| **14.2** | Sitemap and robots configuration — new root `robots.txt` (Allow public storefront; Disallow `admin/`, `api/`, `uploads/`, `config/`, `includes/`, `database/`, `docs/`, every private customer page, and action-only `logout.php`) plus public-only `sitemap.xml` and a live `sitemap.php` generator. Shared catalogue in `includes/seo.php` is the single source of truth for public paths; active `product.php?id=N` rows are appended when MySQL is reachable; absolute `<loc>` values prefer `base_url` then the request host (CLI `--write` / `--base=` regenerates the static snapshot). Commit 14.1 noindex list aligned to also cover consultations/logout. Verified: `php -l` clean, sitemap well-formed XML, all 21 listed public files exist on disk, exclusion audit found no private/admin locs. #13 In progress |
| **14.1** | Page-specific SEO metadata — every page already sets a unique `$pageTitle` / `$pageDescription` / `$pageKeywords`; Stage 14.1 completes the SEO head in `includes/header.php`: a scalable brand `favicon.svg` + `site.webmanifest` + `<meta name="theme-color">`; a self-referencing absolute `<link rel="canonical">` and matching `og:url` built by new `customcore_canonical_url()` (uses exact `SCRIPT_NAME` so it is correct at the domain root or in a `~user/subfolder`, strips a configured `base_url` path to avoid duplication, supports a `$pageCanonical` override or `false` to disable); and a `<meta name="robots">` directive that is `index, follow` for public content but `noindex, nofollow` for admin and per-user private pages via centralised `customcore_is_noindex_page()` (no per-page edits). Verified: `php -l` clean, no lint, and rendered public (`about.php`, `index.php` → index/canonical/og:url/favicon set) and admin (`admin/monitoring.php` → noindex + depth-correct `../favicon.svg`/`../site.webmanifest`) pages; canonical unit-tested for root, subfolder, base_url-with-subfolder (no duplication), override, and disabled. #13 In progress |
