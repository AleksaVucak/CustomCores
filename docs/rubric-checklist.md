...
| 10a | About ~20 dynamic HTML/PHP pages | 4 | Purposeful `.php` pages listed in `docs/sitemap.md` | Count distinct purposeful dynamic pages; no empty placeholder pages | 1–9, 13 | Complete — **51** purposeful dynamic PHP files verified (31 public/customer including `privacy.php` + SEO `sitemap.php` + 17 `admin/*.php` + 3 `api/*.php`), far above the ~20 minimum; public + Help pages return HTTP 200 and private/admin pages correctly gate to login (303) — no empty placeholders. All admin tooling live: dashboard, products, options, compatibility, orders, users, consultations, reviews, reports, themes, monitoring; recorded in [`rubric-audit.md`](rubric-audit.md) |
...
| -- | --------- | ---------------- | ------------ | ------ |
| B1 | HTML5, CSS, JavaScript front end with full interactive functionality | Entire public/customer UI | 1–8 | Complete — storefront and customer flows built with HTML5, external CSS/JS modules (builder, cart, checkout, map, charts, help-hub), and full interactive catalogue/builder/account features |
| B2 | Multimedia: images, video/audio, interactive map, interactive menus, data visualization/graphs | `media.php`, `store-locations.php`, nav, Chart.js (or equivalent) on public + admin reports + builder chart | 8.x, 5.8, 9.9 | Complete — images (8.1) + Learning Centre video/audio (8.2–8.3) + Leaflet/OSM map (8.4) + catalogue chart from MySQL (8.5) + accessible fallbacks (8.6) + media credits (8.7); builder performance chart (5.8); admin report charts with accessible tables (9.9); responsive main menu (B15) |
| B3 | Minimum 20 unique dynamic pages and minimum 5 static pages | Dynamic PHP set + static Help wiki (7 pages) — `docs/sitemap.md` | Throughout; Help in 11.x | Complete — **51** purposeful dynamic PHP files (31 public/customer including privacy/SEO routes + 17 admin + 3 API) and **7** Help/training HTML pages; counts verified under #10a and `docs/sitemap.md` |
...
| B9 | Admin user documentation | `docs/administrator-guide.md` | 12.2 | Complete — `docs/administrator-guide.md` covers admin access, the live dashboard, and every shipped tool (products, options, compatibility, orders, users, consultations, reviews, reports, themes, monitoring) with actions and safety rules |
...
| B12 | PHP functionality for dynamic pages | All catalogue/account/admin PHP | 3–9 | Complete — catalogue, product detail, builder APIs, cart/checkout, accounts, consultations, and the full admin back office are implemented as real PHP against MySQL |
| B13 | Software repository (e.g. GitHub) with code history | GitHub remote on `main`; meaningful commits | 0.1 ongoing | Complete — project history on GitHub `main` with staged incremental commits (Stages 0–16 delivered; Stage 17 documentation polish ongoing) |
