...
**Last updated:** Stage 14 / Commit 14.1 (page-specific SEO metadata — favicon, canonical, robots)
...
| 13 | SEO-friendly meta: icon, title, description, keywords, etc. | 4 | Per-page metadata in layout/header; favicon; `sitemap.xml`; `robots.txt`; semantic HTML | Important public pages have unique title/description; private/admin URLs excluded from sitemap | 14.1–14.3 | In progress — per-page unique title/description/keywords across all pages; SVG favicon (`favicon.svg`) + `site.webmanifest` + `theme-color`; self-referencing absolute `<link rel="canonical">` + `og:url` (subfolder-safe via `customcore_canonical_url()`); `<meta name="robots">` auto-noindex for admin + private per-user pages (`customcore_is_noindex_page()`) — all wired centrally in `includes/header.php` (14.1). `sitemap.xml`/`robots.txt` (14.2) and semantic-HTML pass (14.3) remain |
