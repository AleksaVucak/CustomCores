...
| 10d | At least 20 copyright-free images | 4 | `assets/images/` (≥ 20 files); credits in `docs/media-credits.md` | Images load; filenames meaningful; alt text present; licences documented | 8.1, 8.7 | In progress — 8.1: 20 product images + hero, category, UI, OG, poster assets added and wired via `customcore_image_url()` with graceful fallback; alt text present. Licence credits pending (8.7) |
| 10e | At least 3 video or audio files | 4 | `assets/media/` (≥ 3 items); Learning Centre `media.php` | All three play with browser controls; documented in credits | 8.2–8.3 | In progress — 8.2–8.3: 2× MP4 + 1× MP3 play via native controls; `media.php` is an organized, responsive Learning Centre (lesson directory, badges, captions, transcripts); licence credits pending (8.7) |
...
| B2 | Multimedia: images, video/audio, interactive map, interactive menus, data visualization/graphs | `media.php`, `store-locations.php`, nav, Chart.js (or equivalent) on public + admin reports + builder chart | 8.x, 5.8, 9.9 | In progress — images (8.1) + playable media & Learning Centre `media.php` (8.2–8.3) + interactive Leaflet/OSM map with text fallback `store-locations.php` (8.4) + public catalogue data visualization from live MySQL with accessible table fallback `catalogue.php` (8.5, Chart.js via `catalogue-chart.js`) + accessible multimedia fallbacks statement `accessibility.php` documenting alt/transcripts/captions/chart tables/map address, homepage teaser transcript link, and `prefers-reduced-motion` (8.6); builder performance chart (5.8); admin report charts (9.9) & media credits (8.7) still pending |
