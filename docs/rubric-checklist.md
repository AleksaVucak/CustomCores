...
**Last updated:** Stage 13 / Commit 13.1 (application health-check engine — `includes/monitoring.php`)
...
| B10 | Backend monitoring page (online / warning / offline for site and feature services) | `admin/monitoring.php` + health checks in `includes/monitoring.php` | 13.x | In progress — health-check engine complete (`includes/monitoring.php`, 13.1): seven self-contained checks (PHP runtime, database, sessions, core files, upload storage, site theme, Learning Centre media) each return a controlled online/warning/offline status with production-safe messages, aggregated by `customcore_monitoring_run()` (cannot throw). Admin dashboard page (`admin/monitoring.php`, 13.2), statistics (13.3), error hardening (13.4), and troubleshooting docs (13.5) remain |
