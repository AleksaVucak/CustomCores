...
| 11 | Website available online live (preferably `myweb.cs.uwindsor.ca`) | 2 | Production URL recorded in README; production requirements + deployment docs in `docs/` | Homepage loads publicly without PHP fatals; core flows work on host | 16.x | In progress — host requirements documented in [`production-requirements.md`](production-requirements.md) (PHP, MySQL, permissions, browsers, myweb match); install + deploy guides ready; **live-load test still blocked until the public host is online** |
...
| B14 | Installation documentation for another server | `docs/production-requirements.md`, `docs/installation-guide.md`, `docs/deployment-troubleshooting.md` | 12.4–12.5, 16.1, 16.x | In progress — production host requirements checklist ([`production-requirements.md`](production-requirements.md)), clean-checkout install guide (`installation-guide.md`), and shared-hosting deployment + troubleshooting + backup/rollback guide (`deployment-troubleshooting.md`) published; final live confirmation still pending the hosted site |
