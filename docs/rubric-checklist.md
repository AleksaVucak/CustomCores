...
**Last updated:** Stage 13 / Commit 13.4 (production-safe monitoring error messages — no passwords, paths, or stack traces)
...
| B10 | Backend monitoring page (online / warning / offline for site and feature services) | `admin/monitoring.php` + health checks in `includes/monitoring.php` | 13.x | In progress — health-check engine (13.1) + admin dashboard with online/warning/offline table (13.2) + live statistics (13.3) + error hardening (13.4: `customcore_monitoring_safe_message()` strips passwords, absolute paths, and stack traces from every dynamic error string — DB check, stats panel, and page fallback — even in debug; render-verified with the DB offline that output contains no `/Users/` path, no stack trace, and no password fragment). Troubleshooting docs (13.5) remain |
