...
**Last updated:** Stage 13 / Commit 13.2 (administrator monitoring dashboard — `admin/monitoring.php`)
...
| B10 | Backend monitoring page (online / warning / offline for site and feature services) | `admin/monitoring.php` + health checks in `includes/monitoring.php` | 13.x | In progress — health-check engine (`includes/monitoring.php`, 13.1) + administrator dashboard live (`admin/monitoring.php`, 13.2): seven self-contained checks (PHP runtime, database, sessions, core files, upload storage, site theme, Learning Centre media) each return a controlled online/warning/offline status with production-safe messages, aggregated by `customcore_monitoring_run()` (cannot throw). The dashboard renders an overall banner + per-service status table behind `customcore_require_admin()` and loads even when a check fails (verified with DB offline). Statistics (13.3), error hardening (13.4), and troubleshooting docs (13.5) remain |
