...
| 3a | At least 3 different site-wide CSS templates (distinct look/layout) | 12 | `assets/themes/rgb-gaming.css`, `assets/themes/minimal-pro.css`, `assets/themes/cyber-grid.css` | Themes differ in colour, typography, nav, buttons, cards, spacing, borders, and layout treatment | 10.1–10.3 | Complete (files) — all three distinct templates shipped: **RGB Gaming (10.1)** dark/neon (Space Grotesk, rounded, glow), **Minimal Professional (10.2)** light editorial (Fraunces/Manrope, hairline, flat, blue), **Cyber Grid (10.3)** HUD grid (Orbitron/Chakra Petch/Share Tech Mono, zero-radius, corner-cut buttons, mint+magenta); each skins public + admin via shared tokens |
| 3b | Ability to change the template dynamically | 4 | Admin `admin/themes.php`; MySQL `themes` / `site_settings`; theme loaded in shared header include | Admin selects theme → setting saved → public and admin pages load the chosen CSS | 2.6, 10.1, 10.4–10.5 | In progress — theme + settings rows seeded (2.6); shared header now resolves the active theme from MySQL (`site_settings.active_theme_id → themes.css_file`) with a path-validated fallback via `includes/theme.php` (10.1); admin switcher UI in 10.4 |
...
| 12 | Advanced appropriate CSS (fonts, menus, boxes/cards, transitions, layouts) | 4 | Base CSS + three themes demonstrating typography, nav, cards, transitions, grids, form states | Visual review on desktop and mobile across themes | 1.5, 10.x, 14.5 | In progress — base foundation in `main.css` (1.5); RGB Gaming theme adds gradient wordmark, animated header rail, neon focus/hover states, dark form fields (10.1); remaining themes later |
