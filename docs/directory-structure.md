...
| `docs/` | Business case, sitemap, wireframes, ER design, database import, media credits, image prompts, theme testing, Help context-link audit, production server requirements, production configuration (secrets and paths), installation / administrator / content / deployment guides, monitoring troubleshooting, security audit, repository cleanup, documentation finalization, and the QA records (HTML, CSS, JavaScript, desktop and mobile responsiveness, customer and administrator workflows, production customer/admin workflow records, final defect resolution) |
