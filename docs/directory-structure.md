...
| `config/` | `database.example.php`, `app.php`; real `database.php` gitignored |
| `database/` | Schema, seeds, create-admin script |
| `docs/` | Business case, rubric checklist, sitemap, wireframes, ER design, database import, media credits, image prompts, theme testing, Help context-link audit, production server requirements, installation / administrator / content / deployment guides, monitoring troubleshooting, security audit, and the QA records (HTML, CSS, JavaScript, desktop and mobile responsiveness, customer and administrator workflows, rubric audit, final defect resolution) |
