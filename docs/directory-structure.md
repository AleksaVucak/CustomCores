...
drove the full **registration → order** journey and **every core customer action** (auth, catalogue
browse/search/filter/compare, wishlist, builder + save build, saved-build → cart, cart add/update,
checkout, order creation/confirmation/history/details, review, consultation, edit profile + password
change, logout, plus foreign-order rejection). **Result: 41/41 assertions Pass.** **Fixed** an
avoidable defect: ordering a **saved build** threw a SQL error because `order-confirmation.php`'s
snapshot query joined `component_categories` on the non-existent `c.category_id` (correct column is
`component_category_id`), which had silently blocked every order containing a saved build. Disposable
`@example.test` customer used and all test data deleted afterward (store back to 3 seed customers,
0 orders/builds/carts).
...
Chromium tab at **1024 / 1280 / 1440 / 1920 px** with an automated overflow probe across
**40 page states** (15 public, 11 customer, 14 admin), using a disposable customer briefly
promoted to admin for signed-in coverage (account, order, and helper files deleted after).
Result: **40/40 Pass, 0 unintended horizontal overflow, 0 broken layouts, 0 defects** — no CSS
changes. Confirmed the shell caps at **1120 px** (`--cc-width-max`) and centres on wide screens,
and that wide admin tables + the compare table scroll inside their cards (`overflow-x:auto`),
never the page.

**Commit 15.3 complete — verify JavaScript and browser console.**
New [`docs/js-validation.md`](js-validation.md): `node --check` on all 11 `assets/js/*`
modules (**0 failures**); Chromium console + uncaught-error sweep of public, Help,
customer, and admin core pages (**0 application uncaught errors**). **Fixed** incorrect
Leaflet CSS Subresource Integrity in `includes/header.php` (blocked `leaflet.css` and
logged a console error); JS integrity was already correct; both Leaflet tags use
`crossorigin="anonymous"`. Feature scripts verified under their intended load conditions
(builder / cart / checkout / reviews / contact / map / charts / admin-reports / help-hub).

**Commit 15.2 complete — validate CSS files.**
New [`docs/css-validation.md`](css-validation.md): structural + css-tree parse of all five
project stylesheets (**0 parse errors**), plus a custom-property completeness audit.
**Fixed** undeclared design tokens by declaring semantic aliases/soft fills on
`main.css` `:root`, theme soft-fill overrides for dark/light palettes, and a default
`--score: 0` on result bars. Re-run: every `var(--…)` resolves.
