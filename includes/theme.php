<?php
/**
 * CustomCore — Active theme resolver (Stage 10.1, hardened in 10.5).
 *
 * File responsibility:
 ...
 *   defence-in-depth fallback chain so a page is NEVER left unstyled and a bad
 *   value can never smuggle in a foreign file path.
 *
 * Resolution order (first safe, on-disk match wins):
 *   1. site_settings.active_theme_id → themes.css_file   (admin selection)
 *   2. themes.is_active_default = 1  → css_file           (seeded fallback)
 *   3. config/app.php → default_theme slug               (offline fallback)
 *   4. canonical slug "rgb-gaming"                        (hard-coded fallback)
 *   5. first valid assets/themes/*.css on disk           (last-resort scan)
 *
 * Safety guarantees (Stage 10.5):
 ...
 *     ^assets/themes/<slug>.css exactly. This blocks path traversal (../),
 *     absolute paths, subdirectories, query strings, and non-CSS files even if a
 *     row in the database is corrupt or malicious.
 *   - A candidate must also exist on disk before it is linked, so a missing or
 *     renamed file transparently falls through to the next candidate.
 *   - Database access is wrapped in try/catch: if MySQL is unavailable the
 *     resolver still returns a styled theme from config / canonical / scan.
 *   - If somehow no theme file exists at all, the resolver returns null and the
 *     header simply omits the theme link — the site remains styled by the base
 *     assets/css/main.css, which is always linked first.
 *
